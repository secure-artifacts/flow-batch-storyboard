...
const fbDownloadLedgerKey = `fbDownloadLedgerV231267`;
...
async function fbGetDownloadState(e, t, i = ``) {
  ...
  if (n?.state === `preparing`) {
    const preparingAge = Date.now() - Number(n.requestedAt || 0);
    if (n?.id && preparingAge < 3 * 60 * 1e3) return n;
    if (!n?.id && preparingAge < 20 * 1e3) return n;
    return await fbUpdateDownloadLedger(r, {
      id: 0,
      state: `missing`,
      error: `旧的下载准备状态已经失效，允许重新创建下载任务`,
      finishedAt: Date.now(),
    });
  }
...
var s = class {
  ...
  bindMessage() {
    const fbHandleRpcMessage = (e, t, n) => {
      if (t?.tab?.id) {
        if (
          (this.isDebugger &&
    ...
    // Firefox treats sendMessage(extensionId, ...) from the page bridge as an
    // external message, while extension-owned calls arrive on onMessage.
    // Listen on both channels so generation downloads and checkpoint saves use
    // the same RPC implementation without duplicating handlers.
    chrome.runtime.onMessage.addListener(fbHandleRpcMessage);
    chrome.runtime.onMessageExternal?.addListener(fbHandleRpcMessage);
  }
  bindEvent() {
